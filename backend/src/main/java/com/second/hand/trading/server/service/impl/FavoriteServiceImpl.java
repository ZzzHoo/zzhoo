    ...
    public List<FavoriteModel> getAllFavorite(Long userId){
        ...
        if(list.size()>0){
            ...
            for(IdleItemModel idle:idleItemModelList){
                ...
                if(idle.getUserId()!=null) userIdList.add(idle.getUserId());
            }
            // 批量查用户
            List<UserModel> userList = userDao.findUserByList(userIdList);
            Map<Long,UserModel> userMap = new HashMap<>();
            for(UserModel user:userList){
                userMap.put(user.getId(),user);
            }
            for(FavoriteModel i:list){
                IdleItemModel idle = map.get(i.getIdleId());
                if(idle!=null && idle.getUserId()!=null){
