 ...
 * 作用：订单相关业务逻辑实现类，实现订单的创建、查询、更新等核心功能。
 * 主要功能：
 *   - 新增订单（addOrder）并自动下架商品
 *   - 订单超时处理（定时任务）
 *   - 查询订单详情、根据订单号分页查询
 *   - 订单状态流转（支付、取消、发货、收货等）
 *
 * 主要逻辑说明：
 *   - 通过事务和分布式锁保证下单和商品下架的原子性
 *   - 订单创建时校验商品状态，防止重复下单
 *   - 订单超时自动取消，商品重新上架
 */
@Service
...
//        ReentrantLock lock=new ReentrantLock(true);
    ...
    public OrderModel getOrder(Long id){
        ...
        IdleItemModel idleItem = idleItemDao.selectByPrimaryKey(orderModel.getIdleId());
        if (idleItem != null) {
            idleItem.setUser(userDao.selectByPrimaryKey(idleItem.getUserId()));
        }
        orderModel.setIdleItem(idleItem);
        return orderModel;
    }
    ...
    public boolean updateOrder(OrderModel orderModel){
        ...
        if(orderModel.getOrderStatus()==4){
            ...
            if(idleItemModel.getIdleStatus()==2){
                ...
                if(orderDao.updateByPrimaryKey(orderModel)==1){
                    if(idleItemDao.updateByPrimaryKeySelective(idleItem)==1){
                        return true;
                    }else {
                        new RuntimeException();
        ...
        return orderDao.updateByPrimaryKey(orderModel)==1;
    }
    ...
    @Override
    public boolean updateOrderPaymentStatus(Long orderId, int paymentStatus, String paymentWay) {
        OrderModel orderModel = new OrderModel();
        orderModel.setId(orderId);
        orderModel.setPaymentStatus((byte)paymentStatus);
        orderModel.setPaymentWay(paymentWay);
